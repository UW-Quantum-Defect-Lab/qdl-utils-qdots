...
class ZHL4240Wplus:
    ...
    def raise_for_power(self, input_power_dbm):
        if input_power_dbm > self.max_allowed_input_power_dbm:
            raise qdlutils.errors.QT3Error(f'{input_power_dbm} dbm exceeds maximum allowed power for this device ({self.max_allowed_input_power_dbm} dbm)')
