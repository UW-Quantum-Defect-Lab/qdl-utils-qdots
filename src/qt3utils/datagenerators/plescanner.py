...
class PleScanner:
    ...
    def start(self) -> None:
        ...
        """
        self.running = True
        for rate_counter in self.readers:
            self.readers[rate_counter].start()
    ...
    def sample_count_rate(self, reader, data_counts=None) -> np.floating:
        """
        Sum the counts in a batch
        """
        if data_counts is None:
            data_counts = self.sample_counts(self.readers[reader])
        return self.readers[reader].sample_count_rate(data_counts)
